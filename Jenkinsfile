pipeline {
   ...
   environment {
     ...
     // REPOSITORY_GITHUB
     REPOSITORY_TAG="${ORGANIZATION_NAME}/${REPOSITORY_GREEN}:${BUILD_ID}"
     registry = "${ORGANIZATION_NAME}/${REPOSITORY_GREEN}"
   ...
   stages {
      stage('Preparation') {
         steps {
            cleanWs()
            git credentialsId: 'GitHub', url: "https://github.com/${ORGANIZATION_NAME}/${REPOSITORY_GREEN}"
         }
      }
